...
import { View, FlatList, Image, StyleSheet, Text, Dimensions, TouchableOpacity, ActivityIndicator, SafeAreaView, Linking } from 'react-native';
...
const ApartmentsILiked = ({ apartments, loading, error, onApartmentPress }) => {
  ...
  if (!apartments || apartments.length === 0) {
    return (
      <View style={styles.placeholderContainer}>
        <Text style={styles.placeholderText}>You haven't liked any apartments yet.</Text>
      </View>
    );
  }
...
const PeopleLikedMyApartment = ({ users, loading, error, onUserPress }) => {
  if (loading) {
    return (
      <View style={styles.placeholderContainer}>
        <ActivityIndicator size="large" color="#0000ff" />
        <Text style={styles.hebrewText}>טוען משתמשים שאהבו את הדירה שלך...</Text>
      </View>
    );
  }
    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <View style={styles.userCardWrapper}>
          <UserDisplayer 
            avatarSource={item.profile_image || '../assets/icons/avi-avatar.jpg'}
            name={item.name}
      ...
      <View style={styles.tabContainer}>
        ...
        >
          ...
          >
            ...
            />
            <Text style={[styles.tabText, activeTab === 'apartments' && styles.activeTabText]}>
              Apartments I Liked
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
